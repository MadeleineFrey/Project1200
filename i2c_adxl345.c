...
bool i2c_transmitt(uint8_t data) {
	i2c_wait();
...
void adxl_init (void){
    ...
    i2c_transmitt(DATA_FORMAT);
    i2c_transmitt(0x0); //+-2 g resolution 10 bit mode
    i2c_stop();
...
/*Set the baud rate*/
    ...
    i2c_transmitt(BW_RATE);
    i2c_transmitt(0x0A); // 100 Hz datarate
    i2c_stop();
...
/*Set offset for the y-axis*/
    ...
    i2c_transmitt(OFSY);
    i2c_transmitt(0x20); // 1 and 255. 0 is idle
    i2c_stop();
...
void xaxis_data (int * data){
		...
		i2c_transmitt(DATAX0);

		do {
			i2c_start();
		} while(!i2c_transmitt((ADXL345_ADDRESS << 1) | 1)); 
